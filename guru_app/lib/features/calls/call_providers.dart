...
class CallNotifier extends StateNotifier<CallState>
  ...
  Future<void> initCall(String requestId) async {
    ...
    try {
      ...
      final role = user.role == UserRole.member ? roomMeta.hmsRoleMember : roomMeta.hmsRoleTrainer;
      ...
      DevLogger.instance.log('[RTC]', 'Pre-join ready for room ${roomMeta.hmsRoomId}');
    } catch (e) {
  ...
  Future<void> joinCall() async {
    ...
    try {
      ...
      state = state.copyWith(phase: CallPhase.error, error: e.toString());
      DevLogger.instance.error('[RTC]', 'joinCall failed: $e');
    }
  }
  ...
  Future<void> _writeSessionLog() async {
    ...
    final callService = _ref.read(callServiceProvider);
    final req = await callService.getRequest(_currentRequestId!);
    if (req == null) return;
    ...
    DevLogger.instance.log('[RTC]', 'Session log written: ${log.durationSec}s id=$logId');
  }
  ...
  void onPeerListUpdate({required List<HMSPeer> addedPeers, required List<HMSPeer> removedPeers}) {
    ...
    for (final p in removedPeers) {
      updated.removeWhere((e) => e.peerId == p.peerId);
    }
    for (final p in addedPeers) {
      if (!p.isLocal) updated.add(p);
    }
  ...
  void onPeerUpdate({required HMSPeer peer, required HMSPeerUpdate update}) {
    if (update == HMSPeerUpdate.peerJoined && !peer.isLocal) {
      ...
      state = state.copyWith(
        remotePeers: state.remotePeers.where((p) => p.peerId != peer.peerId).toList(),
      );
    }
  }
  ...
  void onHMSError({required HMSException error}) {
    DevLogger.instance.error('[RTC]', 'HMS error: ${error.message}');
  ...
  @override
  void onReconnecting() {
    DevLogger.instance.warn('[RTC]', 'Reconnecting...');
    state = state.copyWith(phase: CallPhase.joining);
  }

  @override
  void onReconnected() {
    DevLogger.instance.log('[RTC]', 'Reconnected');
  ...
  void onException({HMSActionResultListenerMethod methodType = HMSActionResultListenerMethod.unknown, Map<String, dynamic>? arguments, required HMSException hmsException}) {
    DevLogger.instance.error('[RTC]', 'Action exception: ${hmsException.message}');
  }
