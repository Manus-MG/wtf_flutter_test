...
final unreadCountProvider = StreamProvider.family<int, String>((ref, chatId) {
  ...
  final field =
      user.role == UserRole.member ? 'unreadCountMember' : 'unreadCountTrainer';
  return FirebaseFirestore.instance
      .collection('chats')
      .doc(chatId)
      .snapshots()
  ...
  Future<void> sendMessage() async {
    ...
    try {
      ...
      DevLogger.instance.log(
        '[CHAT]',
        'Message sent: ${msg.id.substring(0, 8)}${uploadedAttachments.isEmpty ? '' : ' (${uploadedAttachments.length} attachment${uploadedAttachments.length == 1 ? '' : 's'})'}',
      );
